...
!***********************************************************************************************************************************
    ...
    TPPLANT=0.0
      ...
      KT = KTWB(JW)
        ...
        IF (CONTOUR(JW)) THEN
        IF (JDAY+(DLT/DAY) >= NXTMCP(JW) .OR. JDAY+(DLT/DAY) >= CPLD(CPLDP(JW)+1,JW))THEN
            IF(VOLUME_BALANCE(JW))THEN
      ...
      IF (MASS_BALANCE(JW)) THEN
        DO JB=BS(JW),BE(JW)
          ...
          IF(DERIVED_CALC)THEN
              DO I=CUS(JB),DS(JB)
              DO K=KT,KB(I)
                TPWB  =  TPWB  + TP(K,I)    *VOL(K,I)/1000.   ! kg
                TPSED =  TPSED + SEDP(K,I)  *VOL(K,I)/1000.   ! kg        
                TNWB  =  TNWB  + TN(K,I)    *VOL(K,I)/1000.   ! kg
                TNSED =  TNSED + SEDN(K,I)  *VOL(K,I)/1000.   ! kg     
                PFLUXIN(JW) = PFLUXIN(JW) + SEDPINFLUX(K,I)*VOL(K,I)/1000.   ! kg
                NFLUXIN(JW) = NFLUXIN(JW) + SEDNINFLUX(K,I)*VOL(K,I)/1000.   ! kg
                DO M=1,NMC
                TNPLANT=TNPLANT+ MAC(K,I,M)*VOL(K,I)*MN(M)/1000.
                TPPLANT=TPPLANT+ MAC(K,I,M)*VOL(K,I)*MP(M)/1000.
                ENDDO
                DO M=1,NEP
...
! END MACROPHYTES
        ...
        IF (JDAY+(DLT/DAY) >= NXTMCP(JW) .OR. JDAY+(DLT/DAY) >= CPLD(CPLDP(JW)+1,JW))WRITE(MASSBFN,'(F10.3,",",1X,I3,",",30(E16.8,",",1X))')JDAY,JW,TPWB,TPSED,TPPLANT,TPOUT(JW),TPTRIB(JW),TPDTRIB(JW),TPWD(JW),TPPR(JW),TPIN(JW),TP_SEDSOD_PO4(JW),PFLUXIN(JW),TNWB,TNSED,TNPLANT,TNOUT(JW),TNTRIB(JW),TNDTRIB(JW),TNWD(JW),TNPR(JW),TNIN(JW),TN_SEDSOD_NH4(JW),NFLUXIN(JW)    ! TP_SEDBURIAL(JW),TN_SEDBURIAL(JW),
        END IF  ! CONTOUR INTERVAL FOR WRITING OUT FLOW BALANCE 

...
SUBROUTINE WITHDRAWAL
  USE GLOBAL; USE GEOMC; USE TVDC; USE SELWC; USE LOGICC; USE MAIN, ONLY: DERIVED_CALC, CDN,tdgon, jsg,  nnsg, ndo, jwd, NGN2, NGCTDG, EA   ! cb 1/16/13
...
IF(QSUMJS.GT.0.0)THEN
  ...
  IF(CONSTITUENTS)then                    ! cb 1/16/13
    ...
    if(tdgon)then
      ...
      IF(NGCTDG /= 0)call total_dissolved_gas (1,palt(id),nnsg,jsg,tavg(js,jb),cavg(js,jb,NGN2))     ! n2 GAS
    end if
  end if
  ...
  IF(QSUMWD.GT.0.0)THEN
    ...
    IF(CONSTITUENTS)then                       ! cb 1/16/13
      ...
      if(tdgon)then
        ...
        IF(NGCTDG /= 0)call total_dissolved_gas (1,palt(i),nnsg,jsg,tavgw(jwd),cavgw(jwd,NGN2))     ! n2 GAS
      end if
    end if
...
!***********************************************************************************************************************************
  ...
  ALLOCATE (VOLM(NWB),MONCTR(NUMTEMPC),NCOUNTCW(NWD),QWDFRAC(NWD),QSTRFRAC(NST,NBR),DYNSEL(NUMTEMPC),SELD(NUMTEMPC),NXSEL(NUMTEMPC),TEMP2(NUMTEMPC))       
      ...
      READ(NUNIT,'(8X,A8,I8,A8,F8.0,F8.0,F8.0,I8,2I8,A8)')TSPLTCNTR(J),TSPLTJB(J),TSYEARLY(J),TSTSRT(J),TSTEND(J),TSPLTT(J),NOUTS(J),(JSTSPLTT(J,N),N=1,2),ELCONTSPL(J)

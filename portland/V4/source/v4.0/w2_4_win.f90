...
IF(END_RUN.or.ERROR_OPEN)STOP    ! SW 6/26/15 3/18/16 Added code to prevent a thread from reinitializing output files as dialog box is closing...intermittant error
...
! open ancillary control file for fish bioenergetics !mlm bioexp
  ...
  INQUIRE(FILE='fish.npt',EXIST=FISH_PARTICLE_EXIST)    ! SW 4/30/15
...
! Restart data
  ...
  IF (RESTART_IN) THEN
    ...
    if(envirpc == '      ON')THEN
      ...
      READ (CONE,'(//8X,I8)') numclass
      Read (CONE,'(//8x,2(5x,a3,f8.3,f8.3))') VEL_VPR, VEL_INT, VEL_TOP,TEMP_VPR,TEMP_INT,TEMP_TOP
...
!SP CEMA
  if(sediment_diagenesis)then
    ...
    If(CEMARelatedCode .and. IncludeCEMASedDiagenesis)Call ComputeCEMADiagenesisSourceSinks
  end if
!End SP CEMA
...
!***********************************************************************************************************************************
    ...
    DO JW=1,NWB
      ...
      DO JB=BS(JW),BE(JW)
        ...
        IF(DLTADD(JW)=='      ON'.and.ABS(H1(KT,I)-H2(KT,I))/H2(KT,I) > 0.5)THEN
            WRITE (WRN,'(A,F0.3,A,I0,A,F0.3/3(A,F0.3),a,i10,a)') 'Computational warning |h1-h2|/h2>0.25 on Julian day = ',JDAY,' at segment ',I,' timestep DLT= ',DLT,&
          ...
          ELSEIF (H1(KT,I) < 0.0) THEN
            ...
            ELSE
              ...
              WRITE(W2ERR,*)'Branch #:',jb,' in Waterbody:',jw
...
220 CONTINUE
    IF (CURMAX < DLT .AND. DLT > DLTMIN) THEN
      ...
      !SP CEMA
      if(sediment_diagenesis)then
        If(CEMARelatedCode .and. IncludeBedConsolidation)TSS       = 0.0
      end if
      !End SP CEMA
      SB        = 0.0
      DLTS      = DLT
...
if(sediment_diagenesis)then
  ...
  If(CEMARelatedCode .and. IncludeBedConsolidation)Call CEMAComputeTurbidity
end if
!End SP CEMA

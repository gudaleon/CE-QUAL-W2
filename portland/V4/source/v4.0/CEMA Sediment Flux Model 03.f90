Subroutine CEMASedimentDiagenesis
    ...
    Real(8) SD_tc, CellArea, CellThickness, FastFluxCarbon, SlowFluxCarbon
    ...
    !  SD_Jnin = nitrogen flux in settling phytoplankton and detritus (gN/m2/d) 
    !  SD_Jpin = phosphorus flux in settling phytoplankton and detritus (gP/m2/d) 
...
!Open Output Snapshot File
	...
	Do JW=1, NWB
        ...
        Do JB=BS(JW),BE(JW)
            ...
            Do SegNumI = IU, ID
		        ...
		        !CellArea = B(KB(SegNumI),SegNumI)*DLX(SegNumI)
    ...
    Entry CEMAMFTRatesandConstants
        ...
        SD_PW_DiffCoeff         =     SDRegnPW_DiffCoeff(RegnNum)
 ...
 ! CEMA testing end
        ...
        do k=ktwb(jw),kb(segnumi)
          SD_Jcin         =  sdinc(k,segnumi)*2.67*h(k,jw)     +SD_Jcin     !  cb 2/28/13 (gC/m3/sec to gO2/m2/sec)
        end do
		...
		SD_Jnin			= 0.d0        
        do k=ktwb(jw),kb(segnumi)
            SD_Jnin			= sdinn(k,segnumi)*h(k,jw) +SD_Jnin
        end do
    ...
    !    SD_Jnin=jnin   ! CEMA testing
		...
		SD_Jpin			= 0.d0
        do k=ktwb(jw),kb(segnumi)
            SD_Jpin			= sdinp(k,segnumi)*h(k,jw) + SD_Jpin
        end do
...
!        SD_PO40     =po4zz/1000.0
    ...
    !reaction velocities (m/d) 
		...
		Do While (.true.) 
		   ...
		   SD_AerLayerThick(SegNumI) = SD_H1
			...
			If (SD_SO4 <= SD_Sulfate_CH4_H2S) Then 
				...
				SD_CSODmax = DMin1((2.0D+00 * SD_KL12 * SD_CH4SAT * SD_JC_O2equiv)**2.0D+00, SD_JC_O2equiv)    ![gmO*/m2-d] = sqr([m/d] * [gmO*/m3] * [gmO*/m2-d])
		...
		If (SD_SO4 < SD_Sulfate_CH4_H2S) Then
			...
			If (SD_CH4CompMethod == 0) Then
				...
				SD_JCH4g = SD_JC_O2equiv - SD_JCH4 - SD_CSOD
				!***********************************************************************
		...
		Call GasBubblesFormation(BubbleRadiusSed(SegNumI), dlt, VolWater)    
		...
		Dissolved_CH4_Src = SD_JCH4*SD_tc/(SD_H1 + SD_H2)   !CHECK CODE
		
		!Dissolved_NH3_Src = (AmmoniaD_SD1*SD_H1 + AmmoniaD_SD2*SD_H2)/(SD_H1 + SD_H2) * PorewaterRelRate(SegNumI)/(CellThickness*CellArea)                      !gm/m³/s
		!Dissolved_NH3_Src = Dissolved_NH3_Src + (SD1_Ammonium*SD_H1 + SD2_Ammonium*SD_H2)/(SD_H1 + SD_H2) * PorewaterRelRate(SegNumI)/(CellThickness*CellArea)  !gm/m³/s
        Dissolved_NH3_Src = (AmmoniaD_SD1*PW_RelRate1 + AmmoniaD_SD2*PW_RelRate2)/(CellThickness*CellArea)                      !gm/m³/d
		Dissolved_NH3_Src = Dissolved_NH3_Src + (SD1_Ammonium*PW_RelRate1 + SD2_Ammonium*PW_RelRate2)/(CellThickness*CellArea)  !gm/m³/d
		
        !Dissolved_H2S_Src = (SulfideD_SD1*SD_H1 + SulfideD_SD2*SD_H2)/(SD_H1 + SD_H2) * PorewaterRelRate(SegNumI)/(CellThickness*CellArea)                      !gm/m³/s
		!Dissolved_H2S_Src = Dissolved_H2S_Src + (SD1_SulfiMinus*SD_H1 + SD2_SulfiMinus*SD_H2)/(SD_H1 + SD_H2) * PorewaterRelRate(SegNumI)/(CellThickness*CellArea)  !gm/m³/s
        Dissolved_H2S_Src = (SulfideD_SD1*PW_RelRate1 + SulfideD_SD2*PW_RelRate2)/(CellThickness*CellArea)                      !gm/m³/d
		Dissolved_H2S_Src = Dissolved_H2S_Src + (SD1_SulfiMinus*PW_RelRate1 + SD2_SulfiMinus*PW_RelRate2)/(CellThickness*CellArea)  !gm/m³/d
		
		!Dissolved_NO3_Src = (SD_NO3(1)*SD_H1 + SD_NO3(2)*SD_H2)/(SD_H1 + SD_H2) * PorewaterRelRate(SegNumI)/(CellThickness*CellArea)                      !gm/m³/s
        Dissolved_NO3_Src = (SD_NO3(1)*PW_RelRate1 + SD_NO3(2)*PW_RelRate2)/(CellThickness*CellArea)                      !gm/m³/d
		
		!Dissolved_SO4_Src = SD_SO4 * PorewaterRelRate(SegNumI)/(CellThickness*CellArea)                      !gm/m³/s
        Dissolved_SO4_Src = SD_SO4 * (PW_RelRate1+PW_RelRate2)/(CellThickness*CellArea)                      !gm/m³/s
        		
		!Dissolved_CO2_Src = (CO2ProducedCon1L1*SD_H1 + CO2ProducedCon2L2*SD_H2)/(SD_H1 + SD_H2) * PorewaterRelRate(SegNumI)/(CellThickness*CellArea)                      !gm/m³/s
        Dissolved_CO2_Src = (CO2ProducedCon1L1*PW_RelRate1 + CO2ProducedCon2L2*PW_RelRate2)/(CellThickness*CellArea)                      !gm/m³/d
        !Dissolved_ALK_Src = (SD_ALK(1)*SD_H1 + SD_ALK(2)*SD_H2)/(SD_H1 + SD_H2) * PorewaterRelRate(SegNumI)/(CellThickness*CellArea)                      !gm/m³/s
        if(IncludeAlkalinity)then
        Dissolved_ALK_Src = (SD_ALK(1)*PW_RelRate1 + SD_ALK(2)*PW_RelRate2)/(CellThickness*CellArea)                      !gm/m³/d
        else
          Dissolved_ALK_Src = 0.0
        end if
        !Dissolved_PO4_Src = (SD_PO4(1)*SD_H1 + SD_PO4(2)*SD_H2)/(SD_H1 + SD_H2) * PorewaterRelRate(SegNumI)/(CellThickness*CellArea)                      !gm/m³/s
        Dissolved_PO4_Src = (SD_PO4(1)*PW_RelRate1 + SD_PO4(2)*PW_RelRate2)/(CellThickness*CellArea)                      !gm/m³/d
        !Dissolved_Fe2_Src = (SD_Fe2(1)*SD_H1 + SD_Fe2(2)*SD_H2)/(SD_H1 + SD_H2) * PorewaterRelRate(SegNumI)/(CellThickness*CellArea)                      !gm/m³/s
        if(IncludeIron)then
        Dissolved_Fe2_Src = (SD_Fe2(1)*PW_RelRate1 + SD_Fe2(2)*PW_RelRate2)/(CellThickness*CellArea)                      !gm/m³/d
        else
          Dissolved_Fe2_Src = 0.0
        end if
        !Dissolved_Mn2_Src = (SD_Mn2(1)*SD_H1 + SD_Mn2(2)*SD_H2)/(SD_H1 + SD_H2) * PorewaterRelRate(SegNumI)/(CellThickness*CellArea)                      !gm/m³/s
        if(IncludeManganese)then
        Dissolved_Mn2_Src = (SD_Mn2(1)*PW_RelRate1 + SD_Mn2(2)*PW_RelRate2)/(CellThickness*CellArea)                      !gm/m³/d
        else
          Dissolved_Mn2_Src = 0.0
        end if
        !Sediment_Heat_Src = SD_rhowcp*(SD_T(1)*SD_H1 + SD_T(2)*SD_H2)/(SD_H1 + SD_H2) * PorewaterRelRate(SegNumI)/(CellThickness*CellArea)                      !J/m³/s
        Sediment_Heat_Src = SD_rhowcp*(SD_T(1)*PW_RelRate1 + SD_T(2)*PW_RelRate2)/(CellThickness*CellArea)                      !J/m³/d
    ...
    Do GenBODNum = 1, NumGenBODConstituents
        ...
        Dissolved_BOD_Src(GenBODNum,1) = -1.d0*SedGenBODConc(GenBODNum,SegNumI,1) * PW_RelRate/CellArea   !gm/m²/d    
        
        !Anaerobic Layer
        PW_RelRate = PorewaterRelRate(SegNumI)*SD_H2/(SD_H1 + SD_H2)*86400.  !m³/d
        Dissolved_BOD_Src(GenBODNum,2) = -1.d0*SedGenBODConc(GenBODNum,SegNumI,2) * PW_RelRate/CellArea   !gm/m²/d    
	...
	Entry ComputeCEMADiagenesisSourceSinks

...
!Specification of when and how often to output information/data
      ...
      IF (M.EQ.1)   OUTFREQ = OUTFREQJAN
      IF (M.EQ.2)   OUTFREQ = OUTFREQFEB
      IF (M.EQ.3)   OUTFREQ = OUTFREQMAR
      IF (M.EQ.4)   OUTFREQ = OUTFREQAPR
      IF (M.EQ.5)   OUTFREQ = OUTFREQMAY
      IF (M.EQ.6)   OUTFREQ = OUTFREQJUN
      IF (M.EQ.7)   OUTFREQ = OUTFREQJUL
      IF (M.EQ.8)   OUTFREQ = OUTFREQAUG
      IF (M.EQ.9)   OUTFREQ = OUTFREQSEP
      IF (M.EQ.10)  OUTFREQ = OUTFREQOCT
      IF (M.EQ.11)  OUTFREQ = OUTFREQNOV
      IF (M.EQ.12)  OUTFREQ = OUTFREQDEC
      IF (JDAY.GT.366) THEN
...
!Calculate Time of Day/Night
      ...
      ELSE
      END IF
      MILTIME = (JDAY-REAL(JDAYG))*24                         ! Military Time - MILTIME used for NFS Calculations
...
! constain random component to segment length and cell layer height
        if(abs(rz).gt.h(fkmp,KTWBF))rz=sign(1.0,rz)*h(fkmp,KTWBF)

SUBROUTINE OUTPUTA  
  ...
  REAL    :: TVOLAVG,QSUMM,CGASD,XDUM,QOUTLET(100),TOUTLET(100),VOLTOT  
...
! code for error checking
    ...
    IF (JDAY.GE.NXTBIO) THEN   
      ...
      DO J=1,NIBIO  
        ...
        DO K = KTWB(JW),KBI(I)  
          ...
          END DO  
	WRITE(BIOEXPFN(J),'(F8.2,",",I8,",",3F8.2,",",<NZOOE-NZOOS+1>(F8.3,","),I8,",",2(F8.2,","),A,",",I0,",",I0)') JDAY,IBIO(J),DEPTHM(K,I),T1(K,I),GAMMA(K,I),&
  ...
  IF (TIME_SERIES) THEN  
    IF (JDAY.GE.NXTMTS.OR.JDAY.GE.TSRD(TSRDP+1)) THEN 
      ...
      write(WLFN,'(f10.3,",",1000(f8.3,","))')jday,((elws(i),i=us(jb),ds(jb)),jb=1,nbr)      
      ...
      DO J=1,NIKTSR  
        ...
        IF (ICE_COMPUTATION) THEN  
          IF(SEDIMENT_CALC(JW))THEN  
            WRITE (TSR(J),'(f10.3,",",18(F10.3,","),1000(A,","))') JDAY,DLT,ELWS(I),T1(K,I),U(K,I),QC(I),SRON(JW)*1.06,GAMMA(K,I),DEPTHB(KB(I),I),    &     ! SW 8/13/06  
            BI(KTWB(JW),I),SHADE(I),ICETH(I),TVOLAVG,rn(i),rs(i),ranlw(jw),rb(i),re(i),rc(i),(ADJUSTR(C2CH(JAC)),JAC=1,NAC),                      &  ! CB 7/26/07
        ...
        ELSE  
          IF(SEDIMENT_CALC(JW))THEN  
            WRITE (TSR(J),'(f10.3,",",17(F10.3,","),1000(A,",")))') JDAY,DLT,ELWS(I),T1(K,I),U(K,I),QC(I),SRON(JW)*1.06,GAMMA(K,I),DEPTHB(KB(I),I),    &     ! SW 8/13/06  
            BI(KTWB(JW),I),SHADE(I),TVOLAVG,rn(i),rs(i),ranlw(jw),rb(i),re(i),rc(i),(ADJUSTR(C2CH(JAC)),JAC=1,NAC),(ADJUSTR(EPCH(JE)),            &  ! CB 7/26/07
    ...
    IF (PROFILE(JW)) THEN  
      IF (JDAY >= NXTMPR(JW) .OR. JDAY >= PRFD(PRFDP(JW)+1,JW)) THEN  
        ...
        elseif(jw == 1)then   ! write out individual files on these days
             ...
             L      = LEN_TRIM(SEGNUM)
             OPEN  (NUNIT,FILE='ProfLongJD'//SEGNUM(1:L)//'.opt',STATUS='UNKNOWN')
             IF(CONSTITUENTS)THEN
             WRITE(NUNIT,'(1000(A,","))')'Seg#','ElevWaterSurf(m)','Q(m3/s)','Temp(oC)','Depth(m)',(CNAME2(CN(JC)),JC=1,NAC),(CDNAME2(CDN(JD,JW)),JD=1,NACD(JW))
             ELSE
             WRITE(NUNIT,'(1000(A,","))')'Seg#','ElevWaterSurf(m)','Q(m3/s)','Temp(oC)','Depth(m)'   
             ENDIF
                 ...
                 DO JJ=1,NWB
                     ...
                     DO JB=BS(JJ),BE(JJ)
                         DO I=CUS(JB),DS(JB)
                             IF(CONSTITUENTS)THEN
                             WRITE(NUNIT,'(I5,",",100(F12.3,","))')I,ELWS(I),QC(I),T2(K,I),DEPTHB(KB(I),I),(C2(K,I,CN(JAC))*CMULT(CN(JAC)),JAC=1,NAC),(CD(K,I,CDN(JD,JW))*CDMULT(CDN(JD,JW)),JD=1,NACD(JW))
                             ELSE
                            WRITE(NUNIT,'(I5,",",100(F12.3,","))')I,ELWS(I),QC(I),T2(K,I),DEPTHB(KB(I),I)
                            ENDIF
                         ENDDO
                     ENDDO
                 ENDDO
    ...
    IF (SPREADSHEET(JW)) THEN  
      IF (JDAY >= NXTMSP(JW) .OR. JDAY >= SPRD(SPRDP(JW)+1,JW)) THEN  
        ...
        DO K=KTWB(JW),KBMAX(JW)  
          WRITE (SPR(JW),'(A,",",2(F10.3,","),1000(F10.3,",",A,","))') 'Temperature',JDAY,DEPTHM(K,DS(BS(JW))),                      &  
        ...
        DO JC=1,NAC  
          IF (PRINT_CONST(CN(JC),JW)) THEN  
            ...
            END DO  
            DO K=KTWB(JW),KBMAX(JW)  
              WRITE (SPR(JW),'(A38,",",2(F10.3,","),1000(F10.3,",",A,","))') CNAME3(CN(JC)),JDAY,DEPTHM(K,DS(BS(JW))),                            &  
              (ELWS(ISPR(J,JW))-DEPTHM(K,ISPR(J,JW)),CONV1(K,J),J=1,NISPR(JW))  
            END DO  
          END IF  
        END DO  
        IF (CONSTITUENTS) THEN  
          DO JD=1,NACD(JW)  
            IF (PRINT_DERIVED(CDN(JD,JW),JW)) THEN  
              ...
              END DO  
              DO K=KTWB(JW),KBMAX(JW)  
                WRITE (SPR(JW),'(A38,",",2(F10.3,","),1000(F10.3,",",A,","))') CDNAME3(CDN(JD,JW)),JDAY,DEPTHM(K,DS(BS(JW))),                     &  
                (ELWS(ISPR(J,JW))-DEPTHM(K,ISPR(J,JW)),CONV1(K,J),J=1,NISPR(JW))  
              END DO  
            END IF  
          END DO  
...
9999      FORMAT (100(e13.6,1x))  
                ...
                DO I=CUS(JB),DS(JB)  
                  DO K=KTWB(JW),KB(I)  
                      ...
                      !   TN_SEDBURIAL(JW) = TN_SEDBURIAL(JW) +KFS(K,I,KF_SED_NBURIAL)/1000.
                         TN_SEDSOD_NH4(JW)= TN_SEDSOD_NH4(JW)+KFS(K,I,KF_NH4_SOD)/1000.+KFS(K,I,KF_NH4_SED)/1000.
                         TP_SEDSOD_PO4(JW)= TP_SEDSOD_PO4(JW)+KFS(K,I,KF_PO4_SOD)/1000.+KFS(K,I,KF_PO4_SED)/1000.
                  END DO  
                END DO  
        END DO  
  ENDIF
          ...
          DO JB=BS(JW),BE(JW)  
            DO I=CUS(JB),DS(JB)  
              DO K=KTWB(JW),KB(I)  
                ...
                KFJW(JW,KFCN(JAF,JW)) = KFJW(JW,KFCN(JAF,JW))+DAY*KFS(K,I,KFCN(JAF,JW))/(ELTMF(JW)*1000.)  ! SUM UP FOR ENTIRE WATERBODY
              END DO  
            END DO  
          END DO  
          DO I=1,NISNP(JW)  
  ...
  IF (DOWNSTREAM_OUTFLOW) THEN  
            ...
            IF (DS(JB) == IWDO(J)) THEN  
              ...
              DO JS=1,NSTR(JB)  
                ...
                IF (CONSTITUENTS)then
                  ...
                  WRITE (WDO2(JFILE,3),'(F10.3,",",1000(F10.4,","))') JDAY,(CAVG(JS,JB,CN(JC)),     JC=1,NAC)  
                end if
                IF (DERIVED_CALC)then
                  cdwdo(cdn(1:nacd(jw),jw),j) = cdwdo(cdn(1:nacd(jw),jw),j)+qstr(js,jb)*cdavg(js,jb,cdn(1:nacd(jw),jw))                 ! cb 1/16/13 
                  WRITE (WDO2(JFILE,4),'(F10.3,",",1000(F10.4,","))') JDAY,(CDAVG(JS,JB,CDN(JD,JW)),JD=1,NACD(JW))  
                end if
              ENDDO  
              JSSS(JB)=NSTR(JB)  
            END IF  
   ...
   !     JWD=0                        ! sw 5/17/13
        ...
        DO JJ=1,NWD  
          ...
          IF (IWD(JWD) == IWDO(J)) THEN  
            ...
            IF (CONSTITUENTS)then
              ...
              WRITE (WDO2(JFILE,3),'(F10.3,",",1000(F10.4,","))') JDAY,(CAVGW(JWD,CN(JC)),     JC=1,NAC)  
            end if
            IF (DERIVED_CALC)THEN  
              !DO JW=1,NWB  
              !  IF (IWDO(J) >= US(BS(JW)) .AND. IWDO(J) <= DS(BE(JW))) EXIT  
              ...
              WRITE (WDO2(JFILE,4),'(F10.3,",",1000(F10.4,","))') JDAY,(CDAVGW(JWD,CDN(JD,JW)),JD=1,NACD(JW))  
            ENDIF  
          ENDIF  
        ENDDO  
          ...
          IF(IWDO(J) == IUSP(JS))THEN  
            ...
            !  JWD=JWD+1
              ...
              IF (DERIVED_CALC)then
                ...
                WRITE (WDO2(JFILE,4),'(F10.3,",",1000(F10.4,","))') JDAY,(CDAVGW(JWD,CDN(JD,JWUSP(JS))),JD=1,NACD(JWUSP(JS)))  
              end if
            ELSE  
            !  JSSS(JBUSP(JS))=JSSS(JBUSP(JS))+1
                qwdo(j)           = qwdo(j)            +qsp(js)                 ! cb 1/16/13  
              ...
              IF (CONSTITUENTS)then
                ...
                WRITE (WDO2(JFILE,3),'(F10.3,",",1000(F10.4,","))') JDAY,(CAVG(JSSS(JBUSP(JS)),JBUSP(JS),CN(JC)),     JC=1,NAC)  
              end if
              IF (DERIVED_CALC)then
                ...
                WRITE (WDO2(JFILE,4),'(F10.3,",",1000(F10.4,","))') JDAY,(CDAVG(JSSS(JBUSP(JS)),JBUSP(JS),CDN(JD,JWUSP(JS))),JD=1,NACD(JWUSP(JS)))  
              end if
            ENDIF  
          ENDIF  
        ENDDO  
          ...
          IF(IWDO(J) == IUPU(JS))THEN  
            ...
            !  JWD=JWD+1              
              ...
              IF (DERIVED_CALC)then
                ...
                WRITE (WDO2(JFILE,4),'(F10.3,",",1000(F10.4,","))') JDAY,(CDAVGW(JWD,CDN(JD,JWUPU(JS))),JD=1,NACD(JWUPU(JS)))  
              end if
            ELSE  
            !  JSSS(JBUPU(JS))=JSSS(JBUPU(JS))+1  
                IF(QPU(JS)==0.0)THEN
              ...
              IF (CONSTITUENTS)then
                ...
                WRITE (WDO2(JFILE,3),'(F10.3,",",1000(F10.4,","))') JDAY,(CAVG(JSSS(JBUPU(JS)),JBUPU(JS),CN(JC)),     JC=1,NAC)  
              end if
              IF (DERIVED_CALC)then
                if(pumpon(js))then   ! cb 1/16/13
                  cdwdo(cdn(1:nacd(jw),jw),j) = cdwdo(cdn(1:nacd(jw),jw),j)+qpu(js)*cdavg(jsss(jbupu(js)),jbupu(js),cdn(1:nacd(jwupu(js)),jwupu(js))) 
                end if
                WRITE (WDO2(JFILE,4),'(F10.3,",",1000(F10.4,","))') JDAY,(CDAVG(JSSS(JBUPU(JS)),JBUPU(JS),CDN(JD,JWUPU(JS))),JD=1,NACD(JWUPU(JS)))  
              end if
            ENDIF  
          ENDIF  
        ENDDO  
          ...
          IF(IWDO(J) == IUPI(JS))THEN  
           ...
           !   JWD=JWD+1  
              ...
              IF (DERIVED_CALC)then
                ...
                WRITE (WDO2(JFILE,4),'(F10.3,1000(F10.4,","))') JDAY,(CDAVGW(JWD,CDN(JD,JWUPI(JS))),JD=1,NACD(JWUPI(JS)))  
              end if
            ELSE  
            !  JSSS(JBUPI(JS))=JSSS(JBUPI(JS))+1
                qwdo(j)           = qwdo(j)            +qpi(js)     ! cb 1/16/13
              ...
              IF (CONSTITUENTS)then
                ...
                WRITE (WDO2(JFILE,3),'(F10.3,",",1000(F10.4,","))') JDAY,(CAVG(JSSS(JBUPI(JS)),JBUPI(JS),CN(JC)),     JC=1,NAC)  
              end if
              IF (DERIVED_CALC)then
                ...
                WRITE (WDO2(JFILE,4),'(F10.3,",",1000(F10.4,","))') JDAY,(CDAVG(JSSS(JBUPI(JS)),JBUPI(JS),CDN(JD,JWUPI(JS))),JD=1,NACD(JWUPI(JS)))  
              end if
            ENDIF  
          ENDIF  
        ENDDO  
        DO JS=1,NGT  ! gates
          ...
          !    JWD=JWD+1  
              ...
              IF (DERIVED_CALC)then
                ...
                WRITE (WDO2(JFILE,4),'(F10.3,",",1000(F10.4,","))') JDAY,(CDAVGW(JWD,CDN(JD,JWUGT(JS))),JD=1,NACD(JWUGT(JS)))  
              end if
            ELSE  
           !   JSSS(JBUGT(JS))=JSSS(JBUGT(JS))+1
               qwdo(j)           = qwdo(j)            +qgt(js)     ! cb 1/16/13
              ...
              IF (CONSTITUENTS)then
                ...
                WRITE (WDO2(JFILE,3),'(F10.3,",",1000(F10.4,","))') JDAY,(CAVG(JSSS(JBUGT(JS)),JBUGT(JS),CN(JC)),     JC=1,NAC)  
              end if
              IF (DERIVED_CALC)then
                ...
                WRITE (WDO2(JFILE,4),'(F10.3,",",1000(F10.4,","))') JDAY,(CDAVG(JSSS(JBUGT(JS)),JBUGT(JS),CDN(JD,JWUGT(JS))),JD=1,NACD(JWUGT(JS)))  
              end if
            ENDIF  
          ENDIF  
        ENDDO  
        ...
        END DO  
          WRITE (WDO(J,1),'(F10.3,",",F9.3,",",8X,100(F9.3,","))') JDAY, QWDO(J), (QOUTLET(I),I=1,NUMOUTLETS)  
          WRITE (WDO(J,2),'(F10.3,",",F8.2,",",8X,100(F8.2,","))') JDAY, TWDO(J), (TOUTLET(I),I=1,NUMOUTLETS)  
          IF (CONSTITUENTS) WRITE (WDO(J,3),'(F10.3,",",1000(A10,","))') JDAY,(CWDOC(CN(JC)),     JC=1,NAC)  
          IF (DERIVED_CALC) WRITE (WDO(J,4),'(F10.3,",",1000(A10,","))') JDAY,(CDWDOC(CDN(JD,JW)),JD=1,NACD(JW))  
      END DO  
    END IF  
  END IF  

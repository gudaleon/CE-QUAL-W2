...
!******** External sources/sinks
         ...
         IF(MASS_BALANCE(JW).AND.CONTOUR(JW).AND.DERIVED_CALC)THEN     ! TO COMPUTE TP AND TN INFLOWS AND OUTFLOWS FOR MASSBAL.OPT FILE
            IF (TRIBUTARIES) THEN
              DO JT=1,JTT
                IF (JB == JBTR(JT)) THEN
                  I = ITR(JT)
...
!**** Kinetic fluxes
    ...
    if(sediment_diagenesis)then
      ...
      If(CEMARelatedCode .and. IncludeCEMASedDiagenesis)Call CEMACalculateRiseVelocity
      If(CEMARelatedCode .and. IncludeCEMASedDiagenesis .and. ApplyBubbTurb)Call CEMABubblesReleaseTurbulence
      If(CEMARelatedCode .and. IncludeCEMASedDiagenesis)Call CEMABubblesTransport
      If(CEMARelatedCode .and. IncludeCEMASedDiagenesis)Call CEMABubbWatTransfer
      If(CEMARelatedCode .and. IncludeCEMASedDiagenesis)Call CEMABubblesRelease

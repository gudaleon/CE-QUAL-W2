...
140     CONTINUE  
    ...
    DO JW=1,NWB  
      IF(VOLUME_BALANCE(JW).AND.CONTOUR(JW))THEN  
        OPEN(FLOWBFN,FILE='flowbal.opt',POSITION='APPEND')  
        JDAY1=0.0  
        REWIND (FLOWBFN)  
        READ   (FLOWBFN,'(//)',END=141)  
        DO WHILE (JDAY1 < JDAY)  
          READ (FLOWBFN,'(F10.0)',END=141) JDAY1  
        END DO  
        BACKSPACE (FLOWBFN)  
141     JDAY1=0.0  
        EXIT  
    ...
    DO JW=1,NWB  
        ...
        OPEN(MASSBFN,FILE='massbal.opt',POSITION='APPEND')  
        JDAY1=0.0  
        REWIND (MASSBFN)  
        READ   (MASSBFN,'(//)',END=143)  
        DO WHILE (JDAY1 < JDAY)  
          READ (MASSBFN,'(F10.0)',END=143) JDAY1  
        END DO  
        BACKSPACE (MASSBFN)  
143     JDAY1=0.0  
        EXIT  
    ...
    ENDDO  
    IF(TIME_SERIES)THEN  
      OPEN(WLFN,FILE='wl.opt',POSITION='APPEND')  
      JDAY1=0.0  
      REWIND (WLFN)  
      READ   (WLFN,'(//)',END=142)  
      DO WHILE (JDAY1 < JDAY)  
  ...
  ELSE  
    ...
    DO JW=1,NWB  
      ...
      IF (SPREADSHEET(JW)) THEN  
        ...
        END DO  
        WRITE (SPR(JW),'(A,A,A,1000("Elevation,",A7,","))') 'Constituent,','Julian_day,','Depth,',(SEG(J),J=1,NISPR(JW))  
      END IF  
      IF (CONTOUR(JW)) THEN  
      ...
      DO J=1,NIKTSR  
        ...
        IF (ICE_COMPUTATION) THEN  
          IF(SEDIMENT_CALC(JW))THEN  
            WRITE (TSR(J),'(1000(A,","))') 'JDAY','DLT(s)','ELWS(m)','T2(C)','U(ms-1)','Q(m3s-1)','SRON(Wm-2)','EXT(m-1)',   &  
            'DEPTH(m)','WIDTH(m)','SHADE','ICETH(m)',                         &
            'Tvolavg(C)','NetRad(Wm-2)','SWSolar(Wm-2)','LWRad(Wm-2)','BackRad(Wm-2)','EvapF(Wm-2)','ConducF(Wm-2)',   &
            (CNAME2(CN(JC)),JC=1,NAC),                                           &  
            ('     EPI',JE=1,NEP),('     MAC',JM=1,NMC),'     SED','    SEDP','    SEDN','    SEDC',   &  
            (CDNAME2(CDN(JD,JW)),JD=1,NACD(JW)),(KFNAME2(KFCN(JF,JW)),JF=1,NAF(JW)),('APLIM',JA=1,NAL),('ANLIM',JA=1,NAL),('ALLIM',JA=1,NAL)  
          ELSE  
            WRITE (TSR(J),'(1000(A,","))') 'JDAY','DLT(s)','ELWS(m)','T2(C)','U(ms-1)','Q(m3s-1)','SRON(Wm-2)','EXT(m-1)',   &  
            'DEPTH(m)','WIDTH(m)','SHADE','ICETH(m)','Tvolavg(C)','NetRad(Wm-2)','SWSolar(Wm-2)','LWRad(Wm-2)','BackRad(Wm-2)','EvapF(Wm-2)','ConducF(Wm-2)',  (CNAME2(CN(JC)),JC=1,NAC),                     &  
            ('     EPI',JE=1,NEP),('     MAC',JM=1,NMC),(CDNAME2(CDN(JD,JW)),JD=1,NACD(JW)),(KFNAME2(KFCN(JF,JW)),JF=1,NAF(JW)),('APLIM',JA=1,NAL),('ANLIM',JA=1,NAL),('ALLIM',JA=1,NAL)    ! SW 10/20/15  
          END IF  
        ELSE  
          IF(SEDIMENT_CALC(JW))THEN  !mlm 7/25/06
            WRITE (TSR(J),'(1000(A,","))')'JDAY','DLT(s)','ELWS(m)','T2(C)','U(ms-1)','Q(m3s-1)','SRON(Wm-2)','EXT(m-1)',   &  
            'DEPTH(m)','WIDTH(m)','SHADE','Tvolavg(C)','NetRad(Wm-2)','SWSolar(Wm-2)','LWRad(Wm-2)','BackRad(Wm-2)','EvapF(Wm-2)','ConducF(Wm-2)',   &
            (CNAME2(CN(JC)),JC=1,NAC),                     &  
            ('     EPI',JE=1,NEP),('     MAC',JM=1,NMC),'     SED','    SEDP','    SEDN','    SEDC',   &  
            (CDNAME2(CDN(JD,JW)),JD=1,NACD(JW)),(KFNAME2(KFCN(JF,JW)),JF=1,NAF(JW)),('APLIM',JA=1,NAL),('ANLIM',JA=1,NAL),('ALLIM',JA=1,NAL)   
          ELSE  
            WRITE (TSR(J),'(1000(A,","))') 'JDAY','DLT(s)','ELWS(m)','T2(C)','U(ms-1)','Q(m3s-1)','SRON(Wm-2)','EXT(m-1)',   &  
             'DEPTH(m)','WIDTH(m)','SHADE','Tvolavg(C)','NetRad(Wm-2)','SWSolar(Wm-2)','LWRad(Wm-2)','BackRad(Wm-2)','EvapF(Wm-2)','ConducF(Wm-2)',   &
            (CNAME2(CN(JC)),JC=1,NAC),                     &  
            ('     EPI',JE=1,NEP),('     MAC',JM=1,NMC),(CDNAME2(CDN(JD,JW)),JD=1,NACD(JW)),(KFNAME2(KFCN(JF,JW)),JF=1,NAF(JW)),('APLIM',JA=1,NAL),('ANLIM',JA=1,NAL),('ALLIM',JA=1,NAL)   
          END IF  
        END IF  
    ...
    IF (DOWNSTREAM_OUTFLOW) THEN  
      DO JWD=1,NIWDO  
        ...
        IF (CONSTITUENTS) THEN  
          ...
          WRITE (WDO(JWD,3),'(A,I0//(100(A,",")))') '$Concentration file for segment ',IWDO(JWD),'JDAY',CNAME2(CN(1:NAC))  
        END IF  
        IF (DERIVED_CALC) THEN  
          OPEN  (WDO(JWD,4),FILE='dwo_'//SEGNUM(1:L)//'.opt',STATUS='UNKNOWN')
          WRITE (WDO(JWD,4),'(A,I0//(100(A,",")))') 'Derived constituent file for segment ',IWDO(JWD),'JDAY',CDNAME2(CDN(1:NACD(JW),JW))  
        END IF  
      END DO  
    END IF  
          ...
          IF(IWDO(JWD)==DS(JB) .AND. NSTR(JB) /= 0)THEN  
            ...
            OPEN(WDO2(JFILE,2),FILE='two_str'//SEGNUM(1:L)//'_seg'//SEGNUM2(1:L2)//'.opt',STATUS='UNKNOWN')
              ...
              IF (CONSTITUENTS) THEN  
                ...
                WRITE (WDO2(JFILE,3),'(A//(100(A,",")))') TITLEWITH,'JDAY',CNAME2(CN(1:NAC))  
              ENDIF  
              IF (DERIVED_CALC) THEN  
                WDO2(JFILE,4) = NUNIT; NUNIT = NUNIT+1  
                ...
                DO JW=1,NWB  
                  IF (IWDO(JWD) >= US(BS(JW)) .AND. IWDO(JWD) <= DS(BE(JW))) EXIT  
                END DO  
                WRITE (WDO2(JFILE,4),'(A//(100(A,",")))') TITLEWITH,'JDAY',CDNAME2(CDN(1:NACD(JW),JW))  
              ENDIF  
            ENDDO  
          ENDIF  
        ENDDO  
    ...
    DO JW=1,NWB  
      IF(VOLUME_BALANCE(JW).AND.CONTOUR(JW))THEN  
        OPEN(FLOWBFN,FILE='flowbal.opt',STATUS='UNKNOWN')  
        IF(VOLUME_BALANCE(JW))THEN
            WRITE(FLOWBFN,'(a120)')'JDAY,WB,VOLIN(m3),VOLPR(m3),VOLOUT(m3),VOLWD(m3),VOLEV(m3),VOLDT(m3),VOLTRB(m3),VOLICE(m3),%VOLerror'
        ELSE
            WRITE(FLOWBFN,'(a120)')'JDAY,WB,VOLIN(m3),VOLPR(m3),VOLOUT(m3),VOLWD(m3),VOLEV(m3),VOLDT(m3),VOLTRB(m3),VOLICE(m3)'
        ENDIF
    ...
    ENDDO  
        DO JW=1,NWB  
      IF(MASS_BALANCE(JW).AND.CONTOUR(JW).AND.DERIVED_CALC)THEN  
        OPEN(MASSBFN,FILE='massbal.opt',STATUS='UNKNOWN')  
     WRITE(MASSBFN,'(A390)')'JDAY,WB,TP-Waterbody(kg),TP-Sediment(kg),TP-Plants(kg),OutflowTP(kg),TributaryTP(kg),DistributedTributaryTP(kg),WithdrawalTP(kg),PrecipitationTP(kg),InflowTP(kg),SED+SOD_PRelease(kg),PFluxtoSediments(kg),TN-Waterbody(kg),TN-Sediment(kg),TN-Plants(kg),OutflowTN(kg),TributaryTN(kg),DistributedTributaryTN(kg),WithdrawalTN(kg),PrecipitationTN(kg),InflowTN(kg),SED+SOD_NRelease(kg),NFluxtoSediments(kg)'   ! 'JDAY,WB,TP-Waterbody(kg),TP-Sediment(kg),TP-Plants(kg),OutflowTP(kg),TributaryTP(kg),DistributedTributaryTP(kg),WithdrawalTP(kg),PrecipitationTP(kg),InflowTP(kg),BurialP_SED(kg),SED+SOD_PRelease(kg),TN-Waterbody(kg),TN-Sediment(kg),TN-Plants(kg),OutflowTN(kg),TributaryTN(kg),DistributedTributaryTN(kg),WithdrawalTN(kg),PrecipitationTN(kg),InflowTN(kg),BurialN_SED(kg),SED+SOD_NRelease(kg)'
        EXIT  
      ENDIF  
    ENDDO  
      
    IF(TIME_SERIES)THEN  
      OPEN(WLFN,FILE='wl.opt',STATUS='UNKNOWN')  
      write(WLFN,'("JDAY,",1000("SEG",i4,","))')((i,i=us(jb),ds(jb)),jb=1,nbr)         
    ENDIF  
